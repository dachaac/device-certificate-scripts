...
openssl req -new -config openssl.cnf -sha384 -nodes -subj "/C=FI/O=Service/CN=Service Root CA" -key service-root-ca/service-root-ca.key -passin file:service-root-ca/service-root-ca.key.password -out service-root-ca/service-root-ca.csr
...
openssl req -new -config openssl.cnf -sha384 -nodes -subj "/C=FI/O=Service/CN=Devices CA" -key service-devices-ca/service-devices-ca.key -passin file:service-devices-ca/service-devices-ca.key.password -out service-devices-ca/service-devices-ca.csr
...
openssl req -new -config openssl.cnf -sha384 -nodes -days 23360 -subj "/C=FI/O=Service/CN=Servers CA" -key service-servers-ca/service-servers-ca.key -passin file:service-servers-ca/service-servers-ca.key.password -out service-servers-ca/service-servers-ca.csr
...
openssl req -new -config openssl.cnf -sha384 -nodes -days 23360 -subj "/C=FI/O=Service/CN=localhost" -addext "subjectAltName = DNS:localhost" -key localhost.key -passin file:localhost.key.password -out localhost.csr
